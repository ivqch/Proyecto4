...
# ---
# 
# * Estudiante: **Iván Quirós Chavarría**
# * Carné: **B55626**
# * Grupo: **2**
# 
# ---
...
def modulador(bits, fc, mpp,x):
    ...
    # 3. Inicializar la señal modulada s(t)
    t_simulacion = np.linspace(0, N*Tc, N*mpp) 
    senal_Tx = np.zeros(t_simulacion.shape) # Señales sumadas
    moduladora = np.zeros(t_simulacion.shape)  # señal de información
...
# Creación del vector de tiempo de muestreo  
t_muestreo = np.linspace(0, 0.1,100)

# Rango de valores
A = [1,-1]

# Posibles formas de onda, usando las 4 funciones del tiempo x(t) 
...
plt.plot(t_muestreo, Promedio, lw=6,color='k',label='Promedio funciones')

# Resultado teórico del valor esperado
E = np.mean(senal_Tx)*t_muestreo  # Valor esperado de la señal 
plt.plot(t_muestreo, E, ':', lw=3,color='m',label='Valor teórico')

# Gráfica de las funciones con el promedio calculado y el valor esperado teórico
...
f = np.linspace(0.0,1.0/(2.0*Tm),Nm//2) #Profe puso 2.0 
